...
class IntAddressHandler(
    ...
    override fun read(buffer: MemoryReadWriteBuffer): BlockAddress? {
        ...
        buffer.read(bytes)
        val asLong = FloatingInt.bytesToInt(byteCount, bytes) ?: return null
    ...
    override fun write(buffer: MemoryReadWriteBuffer, value: BlockAddress?) {
        if (value == null || !value.isValid()) {
            val longToBytes = FloatingInt.intToBytes(this.byteCount, InvalidRef)
            buffer.write(longToBytes)
        } else {
            val longToBytes = FloatingInt.intToBytes(this.byteCount, value.start)
            buffer.write(longToBytes)
        }
