...
package org.wtrzcinski.memory.lock
...
class MemoryLockRegistry : AutoCloseable {
    ...
    val bitmapLock: MemoryFileLock get() {
        return newLock(mode = MemoryFileOpenOptions.WRITE_TRUNCATE)
    }

    fun newLock(ref: BlockAddress? = null, mode: MemoryFileOpenOptions): MemoryFileLock {
        var localOffset = ref
        if (localOffset == null) {
            localOffset = BlockAddress.InvalidAddress
        }

        val compute = locks.compute(localOffset.start) { _, value ->
            val lock = value ?: ReadWriteMemoryFileLock(locks = this, start = BlockAddress(ref = localOffset.start))
