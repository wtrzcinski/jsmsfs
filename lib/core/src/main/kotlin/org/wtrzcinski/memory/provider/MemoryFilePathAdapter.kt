...
import org.wtrzcinski.memory.path.FilePath
import org.wtrzcinski.memory.path.SymbolicFilePath
import java.net.URI
...
class MemoryFilePathAdapter(
    ...
    override fun toRealPath(vararg options: LinkOption): MemoryFilePathAdapter {
        return MemoryFilePathAdapter(delegate = delegate.toRealPath(), fileSystem = fileSystem)
    }

    override fun getParent(): MemoryFilePathAdapter? {
        val delegate1 = delegate.parent ?: return null
        return MemoryFilePathAdapter(delegate = delegate1, fileSystem = fileSystem)
    }
    ...
    override fun toUri(): URI {
        ...
        return this.delegate.toUri(provider.scheme)
    }
    ...
    override fun resolve(other: Path): MemoryFilePathAdapter {
        ...
        return MemoryFilePathAdapter(
            delegate = delegate.resolve(other = other.delegate),
    ...
    override fun resolve(opath: String): MemoryFilePathAdapter {
        return MemoryFilePathAdapter(
            delegate = this.delegate.resolve(path = opath),
            fileSystem = fileSystem,
        )
    }
        ...
        if (count == 0) {
            ...
            return MemoryFilePathAdapter(
                delegate = SymbolicFilePath(
                    fileSystem = checkNotNull(this.fileSystem.provider().fileSystem),
    ...
    override fun register(watcher: WatchService, events: Array<out WatchEvent.Kind<*>>, vararg modifiers: WatchEvent.Modifier?): WatchKey {
        if (watcher !is MemoryWatchService) {
